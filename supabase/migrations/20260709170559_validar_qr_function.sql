-- Máquina de estados + validación de QR (regla de negocio no negociable: corre
-- exclusivamente server-side; el panel de la atracción nunca calcula el
-- resultado localmente, solo envía lo escaneado y muestra la respuesta).
--
-- Se implementa como una función de Postgres (RPC), no como una Edge Function
-- separada, por dos motivos: (1) toda la lógica necesita leer/escribir varias
-- tablas de forma atómica dentro de una sola transacción, para que dos
-- escaneos simultáneos del mismo pase no puedan colarse como "válidos" los
-- dos a la vez (una Edge Function haciendo varias consultas separadas tendría
-- ese riesgo); (2) esta máquina no tiene Docker instalado, que es requisito
-- para desplegar Edge Functions con la CLI de Supabase — la función de
-- Postgres se aplica igual que cualquier otra migración, sin esa dependencia.
-- El resultado es el mismo desde el punto de vista del cliente: se llama a
-- través de la API de Supabase (supabase.rpc('validar_qr', ...)) con la
-- publishable key + la sesión del empleado, nunca con la clave secreta en el
-- cliente, y toda la decisión ocurre server-side.
...
declare
  ...
  v_monto numeric;
begin
  -- 0. El que escanea debe ser un empleado activo del partner dueño de esta atracción.
  ...
  -- 1. Buscar el pase por codigo_qr.
  ...
  -- Verificar la firma HMAC contra la guardada al crear el pase (detecta QR manipulado).
  if v_pase.qr_firma is distinct from p_firma then
    return jsonb_build_object('permitido', false, 'motivo', 'codigo_invalido');
  end if;

  -- 2. Estados terminales.
  if v_pase.estado in ('vencido', 'agotado') then
  ...
  -- 3. Activación (dos tiempos: 1 año para activar, luego validez_dias del producto).
  ...
  -- 6. Permitido: snapshot de comisión (congelado, no se recalcula después) + insert.
...
-- Solo usuarios autenticados (empleados con sesión) pueden llamarla; nunca anon.
revoke all on function public.validar_qr(uuid, text, uuid) from public;
grant execute on function public.validar_qr(uuid, text, uuid) to authenticated;
